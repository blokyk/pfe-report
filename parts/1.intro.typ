...
Au sein de cette tâche, de tout abord plutôt simple, se cachaient en réalité multiples dragons : bien que j'aie eu une formation à la compilation pendant ma scolarité, the same cannot be said about caches, certainly not in as much depth. Thus, the first step was to learn more about them, the findings of which I detail in @sec_cache_coherency. Once that knowledge acquired, I started planning the design of it, as laid out in @sec_design; I dive into the specifics of its implementation in tooling (LLVM) in @sec_llvm, and its hardware simulation in @sec_gem5. The initial planning and the difficulties encountered on the way are explored in @planning. Finally, the fruits of my work are presented in @results, with a few closing words in @conclusion.
