...
Le processeur est centré autour du décodeur, qui est généré à la compilation de gem5 à partir d'une description de l'ISA, et de l'unité d'exécution, qui contrôle l'état du processeur, modifie les registres, et déclenche les accès mémoire.
...
En théorie, pour implémenter nos plans originaux de la @sec_design_hw, nous aurions donc besoin d'effectuer les modifications suivantes :
  ...
  + au moment de l'exécution, ajouter un flag spécifique à la requête mémoire pour signaler que le cache doit modifier son comportement
